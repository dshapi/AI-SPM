...
# ── Block / redact rule (single, else-chained) ──────────────────────────────
# Priority: verbatim system-prompt leak > schema disclosure > default allow.
# Else chaining ensures exactly one output value so OPA never raises
# eval_conflict_error when both predicates match the same response.

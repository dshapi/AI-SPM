...
#   1. override_flag_detected    - explicit bypass attempt
#   2. authority_spoof           - fake system/admin channel
#   3. tool_call_detected        - pasted tool invocation
#   4. sensitive_path_referenced - internal path reference
# All four predicates are independent; any one triggers a block. The else
# chain keeps this a single complete-rule definition so OPA does not raise
# eval_conflict_error when multiple predicates match the same prompt.

allow := {
    "decision": "block",
    "reason":   "policy-override flag in user prompt",
    "action":   "deny_prompt",
} if {
    override_flag_detected
} else := {
    "decision": "block",
    "reason":   "authority-token / system-channel spoof",
    "action":   "deny_prompt",
} if {
    authority_spoof
} else := {
    "decision": "block",
    "reason":   "tool-call injection in user prompt",
    "action":   "deny_prompt",
} if {
    tool_call_detected
} else := {
    "decision": "block",
    "reason":   "sensitive path reference in user prompt",
    "action":   "deny_prompt",
} if {
    sensitive_path_referenced
}
